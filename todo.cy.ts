...
describe('1. example to-do app', () => {
  ...
  context('1.4 with a checked task', () => {
    ...
    it('1.4.1 can edit a checked task', () => {
      ...
      cy.contains('Pay electric bill').dblclick().type('{selectall}{backspace}').type('Pay water bill{enter}');
      
      // cy.contains('Pay electric bill').parent().find('input[type=checkbox]').type('Pay water bill{enter}');
    ...
    it('1.4.4 can delete all completed tasks', () => {
      ...
      cy.should('not.exist', `${button}`);
      // cy.get('.todo-list li').should('not.exist', `${button}`);
    });
  });
